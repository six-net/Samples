...
<script>
  import {
    ...
    doSaveOperationGroup,
    doCheckOperationGroupName,
    ...
    methods: {
      ...
      handleCheckGroupName(rule, value, callback) {
        ...
        new Promise(async function (resolve, reject) {
          ...
          const { data: allowUse } = await doCheckOperationGroupName({
            name: vm.form.name,
      ...
      //显示编辑页面
      async showEditOperationGroup(data) {
        if (!data) {
          this.title = '添加分组'
        ...
        let groupData = this.operationGroupDataDict[this.form.parent]
        if (groupData) {
      ...
      save() {
        this.$refs['form'].validate(async (valid) => {
          if (valid) {
            ...
            const { success, msg } = await doSaveOperationGroup(this.form)
            if (success) {
              this.$baseMessage('保存成功', 'success')
              this.$refs['form'].resetFields()
              this.dialogFormVisible = false
              this.$emit('edit-operation-group-callback')
              this.form = this.$options.data().form
            }
            this.pageStatus.closePageLoading()
            return success
