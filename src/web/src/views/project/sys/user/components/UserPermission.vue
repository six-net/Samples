...
<script>
  ...
  import {
    doAddUserPermission,
    doDeleteUserPermission,
    doClearByUser,
  } from '@/api/sys/userPermission'
  ...
  export default {
    ...
    computed: {
      ...
      getTitle() {
        if (
          this.primaryData &&
          this.primaryData.realName &&
          this.primaryData.realName != ''
        ) {
          return `【${this.primaryData.realName}】授予的权限`
        }
        return '用户权限'
      },
      //获取表格高度
    ...
    methods: {
      ...
      async handleQuery(options) {
        ...
        this.pageStatus.showPageLoading('正在加载用户权限...')
      ...
      //显示页面
      async showUserPermission(data) {
        this.initialize()
        if (data) {
          this.primaryData = Object.assign({}, data)
          this.queryForm.userFilter = { ids: [data.id] }
          this.drawer = true
          this.pageStatus.showPageLoading('正在加载用户权限...')
      ...
      async deletePermissionData(dataKeys) {
        ...
        this.pageStatus.showPageLoading('正在移除用户权限...')
        let deleteResult = await doDeleteUserPermission(
      ...
      //还原用户授权
      async handleRestore() {
        this.$baseConfirm('确认要还原用户权限吗?', null, async () => {
          this.pageStatus.showPageLoading('正在还原用户权限...')
      ...
      async confirmSelectedPermissionCallback($event) {
        if ($event && $event.length > 0) {
          let userId = this.queryForm.userFilter.ids[0]
          this.pageStatus.showPageLoading('正在保存用户权限...')
          const { msg, success } = await doAddUserPermission(
