...
namespace EZNEWApp.Domain.Sys.Service.Impl
{
    ...
    {
        readonly IRepository<PermissionGroup> permissionGroupRepository;

        public PermissionGroupService(IRepository<PermissionGroup> permissionGroupRepository)
        {
        ...
        Result<PermissionGroup> AddPermissionGroup(PermissionGroup permissionGroup)
        {
            ...
            long parentGroupId = permissionGroup.Parent;
            PermissionGroup parentGroup = null;
            if (parentGroupId > 0)
            {
                IQuery parentQuery = QueryManager.Create<PermissionGroup>(c => c.Id == parentGroupId);
        ...
        {
            PermissionGroup currentPermissionGroup = permissionGroupRepository.Get(QueryManager.Create<PermissionGroup>(r => r.Id == newPermissionGroup.Id));
            if (currentPermissionGroup == null)
            {
                return Result<PermissionGroup>.FailedResult("没有指定要操作的分组信息");
            }
            //上级
            long newParentGroupId = newPermissionGroup.Parent;
            long oldParentGroupId = currentPermissionGroup.Parent;
            //上级改变后 
            if (newParentGroupId != oldParentGroupId && newParentGroupId != currentPermissionGroup.Id)
            {
                PermissionGroup parentGroup = null;
                if (newParentGroupId > 0)
                {
                    IQuery parentQuery = QueryManager.Create<PermissionGroup>(c => c.Id == newParentGroupId);
                    parentGroup = permissionGroupRepository.Get(parentQuery);
                    if (parentGroup == null)
                    {
                        return Result<PermissionGroup>.FailedResult("请选择正确的上级分组");
                ...
                currentPermissionGroup.Parent = newParentGroupId;
            }
            //修改信息
        ...
        public Result Remove(IEnumerable<long> groupIds)
        {
            ...
            //删除分组信息
            IQuery removeQuery = QueryManager.Create<PermissionGroup>(c => groupIds.Contains(c.Id));
            removeQuery.SetRecurve<PermissionGroup>(c => c.Id, c => c.Parent);
        ...
        public PermissionGroup Get(long groupId)
        {
            ...
            IQuery query = QueryManager.Create<PermissionGroup>(c => c.Id == groupId);
            return Get(query);
        }
        ...
        public List<PermissionGroup> GetList(IEnumerable<long> groupIds)
        {
            ...
            IQuery query = QueryManager.Create<PermissionGroup>(c => groupIds.Contains(c.Id));
        ...
        /// <param name="query">查询对象</param>
        /// <returns></returns>
        PagingInfo<PermissionGroup> GetPaging(IQuery query)
        {
        ...
        public PagingInfo<PermissionGroup> GetPaging(PermissionGroupFilter permissionGroupFilter)
        {
        ...
        public bool Exist(long groupId)
        {
            ...
            IQuery query = QueryManager.Create<PermissionGroup>(c => c.Id == groupId);
            return permissionGroupRepository.Exists(query);
        }
        ...
        public bool ExistName(string name, long excludeId)
        {
            ...
            IQuery query = QueryManager.Create<PermissionGroup>(c => c.Name == name && c.Id != excludeId);
            return permissionGroupRepository.Exists(query);
        }
