...
function BindEvent() {
    ...
    //一级菜单事件
    var liLength = $("#index-menu-first #menu-nav-con>ul>li").length;
    $("#index-menu-first #menu-nav-con>ul>li").each(function (i, ele) {
        var menuGroupItem = $(ele);
        menuGroupItem.find(".menu-list>.funcgroupmenu-list").each(function (i, mlele) {
            var menuCon = $(mlele);
            var menuListTitle = menuCon.prev('.funcgroupmenu-title');
            var menItems = menuCon.find('ul>li');
            if (menItems.length < 1) {
                menuCon.remove();
                menuListTitle.remove();
            }
        });
        var menuItemGroups = menuGroupItem.find(".menu-list>.funcgroupmenu-list");
        if (menuItemGroups.length < 1) {
            menuGroupItem.remove();
        }
    });
    ...
    //清除空的菜单项
    $("#index-head .layui-nav-child dd").each(function (i, e) {
        var content = $.trim($(e).html());
        if (content == '') {
            $(e).remove();
        }
    });
